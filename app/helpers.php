...
if(!function_exists('getuservote'))
{
     ...
     * @return \Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector
     */
    function getuservote($postid,$votingtype,$userid)
    {
        $uservoting_resp = \App\Uservoting_onpost::where(['post_id' => $postid,'user_id' => $userid]);
        // if($uservoting_resp->count() > 0){
        //     $resp= $uservoting_resp->first();
        //     return ($votingtype==$resp->voting_type) 'active' : '';
        // }
        return $uservoting_resp->first();
    }
}